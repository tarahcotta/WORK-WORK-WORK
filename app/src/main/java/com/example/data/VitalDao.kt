...
interface VitalDao {
    ...
    @Query("SELECT COUNT(*) FROM logged_workout_sessions")
    suspend fun getSessionCount(): Int

    @Transaction
    suspend fun seedInitialSessionsIfEmpty(
        sessionsWithSets: List<Pair<LoggedWorkoutSessionEntity, List<LoggedSetEntity>>>
    ) {
        if (getSessionCount() == 0) {
            sessionsWithSets.forEach { (session, sets) ->
                val sessionId = insertSession(session)
                val updatedSets = sets.map { it.copy(sessionId = sessionId) }
                insertSets(updatedSets)
            }
        }
    }
    ...
    @Query("SELECT * FROM logged_sets WHERE exerciseName = :exerciseName ORDER BY id DESC LIMIT 1")
